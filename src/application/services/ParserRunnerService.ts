...
export class ParserRunnerService extends EventEmitter {
  ...
  async run(parserName: string): Promise<void> {
    ...
    // Set sentinel synchronously to block concurrent calls before the first await
    this.activeRuns.set(parserName, null as unknown as ParserOrchestrator)
    try {
      ...
      const onComplete = async (stats: unknown) => {
        ...
        await this.runPersistence
          .markRunCompleted(ref.orchestrator!.runId, await ref.orchestrator!.getAllTasks())
          .catch(console.error)
      ...
      await this.runPersistence
        .create({ parserName, runId: ref.orchestrator.runId })
        .catch(console.error)
  ...
  async resume(parserName: string): Promise<void> {
    ...
    try {
      ...
      if (!latest || latest.status !== 'stopped')
        throw new Error(`No stopped run found for "${parserName}"`)
  ...
  async stop(parserName: string): Promise<void> {
    ...
    if (orchestrator) {
      ...
      await this.runPersistence
        .markRunStopped(runId, await orchestrator.getAllTasks())
        .catch(console.error)
      this.activeRuns.delete(parserName)
  ...
  async forceRun(parserName: string): Promise<void> {
    ...
    if (orchestrator) {
      await orchestrator.stop()
      await this.runPersistence
        .update(orchestrator.runId, { status: 'failed', stoppedAt: new Date() })
        .catch(console.error)
      this.activeRuns.delete(parserName)
    } else {
      const latest = await this.runPersistence.getLatestRunInfo(parserName)
