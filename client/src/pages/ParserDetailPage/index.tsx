...
export function ParserDetailPage() {
  ...
  useEffect(() => {
    setRunsPage(1) // eslint-disable-line react-hooks/set-state-in-effect
  }, [parserId])

  // Resolve parser name once
  useEffect(() => {
    setParserName(null) // eslint-disable-line react-hooks/set-state-in-effect
    getParser(parserId)
      .then((r) => setParserName(r.parser.name))
      .catch(() => {})
  }, [parserId])
  ...
  useEffect(() => {
    setCsvPage(1) // eslint-disable-line react-hooks/set-state-in-effect
    setCsvData(null)
  }, [selectedFile])
  ...
  useEffect(() => {
    ...
    if (!selectedFile.name.endsWith('.csv')) {
      setCsvData(null) // eslint-disable-line react-hooks/set-state-in-effect
      return
    }
    let cancelled = false
    setCsvLoading(true)
          ...
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between gap-2 shrink-0">
            ...
            {files.length > 0 && (
              <div className="flex items-center gap-2 min-w-0">
                ...
                >
                  {files.map((f) => (
                    <option key={`${f.runId}/${f.name}`} value={f.name}>
                      {f.name}
                    </option>
                  ))}
                </select>
                      ...
                      <thead className="bg-gray-50 dark:bg-gray-900/50 sticky top-0">
                        <tr>
                          {csvData.headers.map((h, i) => (
                            <th
                              key={i}
                              className="px-3 py-2 text-left font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap border-b border-gray-100 dark:border-gray-700"
                            >
                              {h}
                            </th>
                          ))}
                        ...
                        {pageRows.map((row, ri) => (
                          <tr key={ri} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            {row.map((cell, ci) => (
                              <td
                                key={ci}
                                className="px-3 py-2 text-gray-600 dark:text-gray-400 max-w-[120px] truncate"
                                title={cell}
                              >
                                {cell}
                              </td>
                    ...
                    {csvTotalPages > 1 && (
                      <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between gap-2 shrink-0 sticky bottom-0 bg-white dark:bg-gray-800">
                        <span className="text-xs text-gray-500 dark:text-gray-400">
                          {csvData.total === 0
                            ? 'No rows'
                            : `${(csvData.page - 1) * csvData.limit + 1}–${Math.min(csvData.page * csvData.limit, csvData.total)} of ${csvData.total} rows`}
                        </span>
                        <div className="flex items-center gap-1">
                          <button
