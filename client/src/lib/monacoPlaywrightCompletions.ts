...
export function registerPlaywrightCompletions(monaco: Monaco): void {
  ...
  const toMonacoItems = (specs: CompletionItemSpec[], range: { startLineNumber: number; startColumn: number; endLineNumber: number; endColumn: number }) =>
    specs.map((s) => ({
      label: s.label,
  ...
  monaco.languages.registerCompletionItemProvider('javascript', {
    triggerCharacters: ['.'],
    provideCompletionItems(model: { getLineContent: (n: number) => string }, position: { lineNumber: number; column: number }) {
