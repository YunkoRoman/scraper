...
export function buildPlaywrightCompletionItems() {
    return PAGE_METHODS;
}
export function buildTaskCompletionItems() {
    return TASK_FIELDS;
}
export function registerPlaywrightCompletions(monaco) {
    const toMonacoItems = (specs, range) => specs.map((s) => ({
        label: s.label,
    ...
    monaco.languages.registerCompletionItemProvider('javascript', {
        ...
        provideCompletionItems(model, position) {
            ...
            if (/\bpage\.$/.test(before))
                return { suggestions: toMonacoItems(PAGE_METHODS, range) };
            if (/\btask\.$/.test(before))
                return { suggestions: toMonacoItems(TASK_FIELDS, range) };
            return { suggestions: [] };
        },
    });
}
//# sourceMappingURL=monacoPlaywrightCompletions.js.map